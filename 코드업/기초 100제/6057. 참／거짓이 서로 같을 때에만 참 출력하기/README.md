# [기초-논리연산] 참/거짓이 서로 같을 때에만 참 출력하기(설명)(py) - 6057

[문제 링크](https://codeup.kr/problem.php?id=6057)

### 성능 요약

시간 제한: 1 Sec, 메모리 제한: 128 MB

### 구분

코드업 > 기초 100제

### 문제 분류

기초-논리연산

### 채점결과

정확한 풀이

### 문제 설명

<span>본 문제는 <span>python</span> 의 빠른 기초 학습을 위해 설계된 문제로서 <span>python</span> 코드 제출을 기준으로 설명되어 있습니다.&nbsp;</span><br />
<span>------</span><br />
<br />
2개의 정수값이 입력될 때&#44; <br />
그 불 값(True/False) 이 서로 같을 때에만 True 를 출력하는 프로그램을 작성해보자. <br />
<p style="text-align:right;">
	<img src="/admin/../upload/pimg6222_1.png" alt="" /> <br />
<br />
<br />

### 입력

2개의 정수가 공백을 두고 입력된다. <br />
<br />

### 출력

두 값의 True / False 값이 서로 같을 경우만 True 를 출력하고&#44; 그 외의 경우에는 False 를 출력한다. <br />
<br />

### 입력 예시

<pre>0 0</pre>

### 출력 예시

<pre>True</pre>

> 출처: [코드업 기초 100제](https://codeup.kr/problemsetsol.php?psid=33)
